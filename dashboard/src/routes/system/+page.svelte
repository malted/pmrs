<script lang="ts">
	...
	onMount(() => {
		const interval = setInterval(async () => {
			data = await fetch(`${base}api/system`).then((res) => res.json());
		}, 5_000);
	...
	$: sys = data.payload;
	const cpuLabel = (freq: number) =>
		`${(freq / 1_000).toFixed(1)}<span class="text-zinc-400">GHz</span>`;
	const gbLabel = (bytes: number) =>
		`${padStartNbsp(
			(bytes / 1_000_000_000).toFixed(2),
			6
		)}<span class="text-zinc-400">GB</span>`;
</script>
...
<section class="w-full flex flex-col items-center gap-4">
	...
	<div class="flex items-start gap-3">
		<div class={`grid grid-template-4-autos gap-x-4 items-center ${panelClass}`}>
			<Progress
				label={sys.global_cpu.brand}
				sublabel={cpuLabel(sys.global_cpu.frequency)}
				value={sys.global_cpu.cpu_usage}
				max={100}
			/>
		...
		<div class="flex flex-col gap-3">
			<div class={`grid grid-template-4-autos gap-x-4 items-center ${panelClass}`}>
				<Progress
					label="Memory"
					sublabel={gbLabel(sys.mem_used)}
					value={sys.mem_used}
					max={sys.mem_total}
				/>
				<Progress
					label="Swap"
					sublabel={gbLabel(sys.swap_used)}
					value={sys.swap_used}
					max={sys.swap_total}
				/>
			</div>
			...
			<div class={`grid grid-template-4-autos gap-x-4 items-center ${panelClass}`}>
				{#each sys.disks as disk}
					<Progress
						label={`${disk.name} (${disk.file_system} ${disk.type})`}
						sublabel={`${gbLabel(disk.total_space - disk.available_space)} / ${gbLabel(
							disk.total_space
						)}`}
